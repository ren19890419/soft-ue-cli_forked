...
def test_forced_port_fallback_uses_different_instance_url(monkeypatch):
    monkeypatch.delenv("SOFT_UE_BRIDGE_URL", raising=False)
    monkeypatch.setenv("SOFT_UE_BRIDGE_PORT", "8081")
    monkeypatch.setattr("soft_ue_cli.discovery._find_project_instance", lambda: "http://127.0.0.1:8080")

    assert get_forced_port_fallback_url("http://127.0.0.1:8081") == "http://127.0.0.1:8080"

def test_forced_port_fallback_ignores_same_instance_url(monkeypatch):
    monkeypatch.delenv("SOFT_UE_BRIDGE_URL", raising=False)
    monkeypatch.setenv("SOFT_UE_BRIDGE_PORT", "8081")
    monkeypatch.setattr("soft_ue_cli.discovery._find_project_instance", lambda: "http://127.0.0.1:8081")

    assert get_forced_port_fallback_url("http://127.0.0.1:8081") is None

def test_forced_port_fallback_does_not_override_full_url(monkeypatch):
    monkeypatch.setenv("SOFT_UE_BRIDGE_URL", "http://custom:9000")
    monkeypatch.setenv("SOFT_UE_BRIDGE_PORT", "8081")
    monkeypatch.setattr("soft_ue_cli.discovery._find_project_instance", lambda: "http://127.0.0.1:8080")
